...
See the [current repository layout](../../README.md). Classic lives in `apps/classic`, its derived data in `data/classic`, and its source corpus in `data/shared`. Authoring tools live in `tools/data`. The sections below retain the original product and research context.
...
- now that three datasets are live, the immediate next product phase is the one documented in `docs/classic/multi-dataset-next-phase-plan.md`
