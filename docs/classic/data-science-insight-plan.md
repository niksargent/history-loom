...
Implemented now:
- a dedicated derived-insight schema in `apps/classic/src/types/insights.ts`
- offline generation pipeline in `tools/data/generate-derived-insights.mjs`
- precomputed dataset insight packs in `data/classic/derived/`
- a cross-dataset affinity pack in `data/classic/derived/cross-dataset.json`
