...
const dataDir = path.join(repoRoot, 'data/shared')
const derivedDir = path.join(repoRoot, 'data/classic/derived')
const publicCopyPath = path.join(repoRoot, 'data/classic/public-copy.json')
...
const publicUiFiles = [
  'apps/classic/src/App.tsx',
  'apps/classic/src/components/DetailPanel.tsx',
  'apps/classic/src/components/ComparePanel.tsx',
  'apps/classic/src/components/ForceExplorer.tsx',
  'apps/classic/src/components/InsightsLabPage.tsx',
  'apps/classic/src/components/LoomCanvas.tsx',
  'apps/classic/src/components/InMotionRacePanel.tsx',
]
...
function validatePublicCopyConfig() {
  scanPublicFields(publicCopy, 'data/classic/public-copy.json')
}

function buildDatasetFilePath(dataPath, fileName) {
  return dataPath === '.'
    ? path.join('data/shared', `${fileName}.json`)
    : path.join('data/shared', dataPath, `${fileName}.json`)
}
...
function validateDatasetRegistry(datasets, themes, geographies) {
  assertUniqueIds(datasets, 'data/shared/datasets.json')
  ...
  for (const entry of datasets) {
    if (dataPaths.has(entry.dataPath)) {
      fail(`data/shared/datasets.json reuses dataPath "${entry.dataPath}".`)
    }
...
function validateDatasetPack(entry, pack, themeIds, geographyIds) {
  ...
  for (const pressure of pressures) {
    ...
    if (!publicCopy.pressureCopy[pressure.id]) {
      fail(`${entry.id} pressure ${pressure.id} is missing public copy in data/classic/public-copy.json.`)
    }
  }
...
async function main() {
  ...
  const datasets = await readJson('data/shared/datasets.json')
  const themes = await readJson('data/shared/themes.json')
  const geographies = await readJson('data/shared/geographies.json')
  const validationRegistry = await readJson('data/shared/validation-registry.json')
  ...
  for (const fileName of datasetDerivedFiles) {
    const pack = await readJson(path.join('data/classic/derived', fileName))
    validateDatasetInsightPack(fileName, pack, datasetContexts)
  }

  for (const entry of datasets) {
    if (!datasetDerivedFiles.includes(`${entry.id}.json`)) {
      warn(`Dataset ${entry.id} has no derived pack in data/classic/derived yet.`)
    }
  }

  const crossDatasetPack = await readJson(path.join('data/classic/derived', 'cross-dataset.json'))
