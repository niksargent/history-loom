...
const datasetInsightLoaders = import.meta.glob('../../../../data/classic/derived/*.json') as InsightModuleLoader<DatasetInsightPack>
const crossDatasetInsightLoaders = import.meta.glob('../../../../data/classic/derived/cross-dataset.json') as InsightModuleLoader<CrossDatasetInsightPack>

export async function getDatasetInsightPack(
  datasetId: string,
): Promise<DatasetInsightPack | null> {
  const loader = datasetInsightLoaders[`../../../../data/classic/derived/${datasetId}.json`]
...
export async function getCrossDatasetInsightPack(): Promise<CrossDatasetInsightPack> {
  if (!crossDatasetInsightPromise) {
    const loader = crossDatasetInsightLoaders['../../../../data/classic/derived/cross-dataset.json']
